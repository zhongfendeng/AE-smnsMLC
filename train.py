...
from models.model import AEsmnsMLC
import torch
import sys
...
def train(config):
    ...
    # build up model
    aesmnsmlc = AEsmnsMLC(config, corpus_vocab, model_mode='TRAIN')
    #aesmnsmlc = torch.nn.DataParallel(aesmnsmlc)
    aesmnsmlc.to(config.train.device_setting.device)
    ...
    optimize = set_optimizer(config, aesmnsmlc)

    # get epoch trainer
    trainer = Trainer(model=aesmnsmlc,
                      criterion=criterion,
                      optimizer=optimize,
                      vocab=corpus_vocab,
                      config=config)
    ...
    if not os.path.isdir(model_checkpoint):
        ...
        for model_file in dir_list[::-1]:
            if model_file.startswith('best_macro'):
    ...
    for epoch in range(config.train.start_epoch, config.train.end_epoch):
        ...
        if performance['micro_f1'] > best_performance[0]:
            ...
            save_checkpoint({
                ...
                'state_dict': aesmnsmlc.state_dict(),
    ...
    if os.path.isfile(best_epoch_model_file):
        load_checkpoint(best_epoch_model_file, model=aesmnsmlc,
                        config=config,
                        optimizer=optimize)
...
if __name__ == "__main__":
    configs = Configure(config_json_file='config/ae-smnsmlc.json')
